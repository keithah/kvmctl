...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("KVMCTL_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "kvmctl-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.KvmctlKvmdToken
	if token == "" {
		return ""
	}
	if c.KvmctlKvmdToken == "" {
		return ""
	}
	return token
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.KvmctlKvmdToken != "" {
		parts = append(parts, "kvmd_token="+c.KvmctlKvmdToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		KvmctlKvmdToken: c.KvmctlKvmdToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.KvmctlKvmdToken == "" {
		c.KvmctlKvmdToken = creds.KvmctlKvmdToken
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.KvmctlKvmdToken = token
	delete(c.envOverrides, "KvmctlKvmdToken")
	c.updateFileConfigField("KvmctlKvmdToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["KvmctlKvmdToken"] {
			out.KvmctlKvmdToken = c.fileConfig.KvmctlKvmdToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "KvmctlKvmdToken":
		c.fileConfig.KvmctlKvmdToken = c.KvmctlKvmdToken
	}
}
