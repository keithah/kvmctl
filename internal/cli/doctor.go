...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kvmctl-pp-cli doctor
  kvmctl-pp-cli doctor --json
  kvmctl-pp-cli doctor --fail-on warn
  kvmctl-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export KVMCTL_KVMD_TOKEN=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("KVMCTL_KVMD_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "KVMCTL_KVMD_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "KVMCTL_KVMD_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "kvmctl-pp-cli", suggestion)
						} else {
