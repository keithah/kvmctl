...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KVMCTL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KVMCTL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kvmctl-pp-cli/feedback")
...
When ` + "`KVMCTL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KVMCTL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  kvmctl-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  kvmctl-pp-cli feedback --stdin < notes.txt
  kvmctl-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kvmctl-pp-cli feedback list
  kvmctl-pp-cli feedback list --limit 5
  kvmctl-pp-cli feedback list --json`,
