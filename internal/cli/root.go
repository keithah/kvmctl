...
type rootFlags struct {
	...
	// invocation. Mirrors the KVMCTL_NO_LEARN env var.
	noLearn bool
...
Run 'kvmctl-pp-cli doctor' to verify auth and connectivity.`,
	...
	rootCmd.SetVersionTemplate("kvmctl-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select ok,result)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'kvmctl-pp-cli profile list')")
	...
	rootCmd.AddCommand(newHidCmd(flags))
	rootCmd.AddCommand(newKvmdCompatibleKvmAuthCmd(flags))
	rootCmd.AddCommand(newStreamerCmd(flags))
	...
	rootCmd.AddCommand(newInfoPromotedCmd(flags))
