---
name: pp-kvmctl
description: "Printing Press CLI for Kvmctl. The verified REST subset used by kvmctl for KVMD-compatible GLKVM devices."
author: "keithah"
license: "Apache-2.0"
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [kvmctl-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/devices/kvmctl/cmd/kvmctl-pp-cli
---

# Kvmctl — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `kvmctl-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install kvmctl --cli-only
   ```
2. Verify: `kvmctl-pp-cli --version`
...
Use this exact non-planning loop only after an operator or external LLM has chosen the intended UI action:

```bash
# The OCR command receives fresh screenshot bytes on stdin.
export KVMCTL_OCR_COMMAND=tesseract
# Default protocol is Tesseract TSV; custom structured commands use JSON.
...
**kvmd-compatible-kvm-auth** — Manage kvmd compatible kvm auth
...
```bash
kvmctl-pp-cli which "<capability in your own words>"
```

`which` resolves a natural-language capability query to the best matching command from this CLI's curated feature index. Exit code `0` means at least one match; exit code `2` means no confident match — fall back to `--help` or use a narrower query.

## Auth Setup
Run `kvmctl-pp-cli auth setup` to print the URL and steps for getting a key (add `--launch` to open the URL). Then set:

```bash
export KVMCTL_KVMD_TOKEN="<your-key>"
```
To persist credentials, use `echo "$TOKEN" | kvmctl-pp-cli auth set-token`. Stored secrets live in `credentials.toml` under the data dir, not in `config.toml`.

Run `kvmctl-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  kvmctl-pp-cli info --agent --select ok,result
  ```
...
- Use `--home <dir>` for one invocation, or set `KVMCTL_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `KVMCTL_CONFIG_DIR`, `KVMCTL_DATA_DIR`, `KVMCTL_STATE_DIR`, `KVMCTL_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `KVMCTL_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `kvmctl-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `KVMCTL_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `KVMCTL_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
kvmctl-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "kvmctl-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `kvmctl-pp-cli playbook amend`
       at end-of-session.
...
```bash
kvmctl-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
`kvmctl-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `KVMCTL_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
kvmctl-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
kvmctl-pp-cli feedback --stdin < notes.txt
kvmctl-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `KVMCTL_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `KVMCTL_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
kvmctl-pp-cli profile save briefing --json
kvmctl-pp-cli --profile briefing info
kvmctl-pp-cli profile list --json
kvmctl-pp-cli profile show briefing
kvmctl-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `kvmctl-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add kvmctl-pp-mcp -- kvmctl-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which kvmctl-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   kvmctl-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `kvmctl-pp-cli <command> --help`.
